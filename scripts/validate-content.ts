...
import { EventSchema, ContentPackSchema, CreationSchema, type Event } from "../engine/schema";
...
const pack = packParsed.success ? packParsed.data : { callbacks: [] as any[], scoreThreads: [] as string[] };

/* ---------- 1bis. Création : les threads qu'elle pose ---------- */

const creationThreads = new Set<string>();
try {
  const cRaw = JSON.parse(readFileSync(join(packDir, "creation.json"), "utf8"));
  const cParsed = CreationSchema.safeParse(cRaw);
  if (!cParsed.success) {
    for (const e of cParsed.error.issues) fail("SCHEMA", `creation.json → ${e.path.join(".")} : ${e.message}`);
  } else {
    for (const step of cParsed.data.steps)
      for (const opt of step.options ?? []) {
        for (const t of opt.effects?.threads ?? []) creationThreads.add(t);
        for (const t of Object.keys(opt.threadChance ?? {})) creationThreads.add(t);
      }
  }
} catch {
  warn("CREATION", "creation.json absent : les threads d'origine seront signalés comme inconnus");
}
...
/** thread -> âge minimum auquel il peut être posé */
const threadPosedAt = new Map<string, number>();
/** thread -> liste des événements qui le posent */
const threadPosedBy = new Map<string, string[]>();
const threadRequired = new Map<string, { event: string; age: number; ageMax: number }[]>();
const threadResolved = new Set<string>();

const noteRequirement = (t: string, event: string, age: number, ageMax?: number) => {
  if (!threadRequired.has(t)) threadRequired.set(t, []);
  threadRequired.get(t)!.push({ event, age, ageMax: ageMax ?? age });
};

for (const e of events) {
  const [minAge, maxAge] = e.ageWindow;

  for (const t of e.requires?.threads ?? []) noteRequirement(t, e.id, minAge, maxAge);
  for (const t of Object.keys(e.requires?.threadLevel ?? {})) noteRequirement(t, e.id, minAge, maxAge);
  for (const t of e.requires?.threadsAbsent ?? []) noteRequirement(t, e.id, minAge, maxAge);

  for (const c of e.choices) {
    for (const t of c.requires?.threads ?? []) noteRequirement(t, `${e.id}.${c.id}`, minAge, maxAge);
    for (const t of Object.keys(c.requires?.threadLevel ?? {})) noteRequirement(t, `${e.id}.${c.id}`, minAge, maxAge);
    ...
    for (const eff of allEffects) {
      for (const t of eff.threads ?? []) {
        ...
        if (prev === undefined || minAge < prev) threadPosedAt.set(t, minAge);
...
/*     C'est ce test qui rattrape l'erreur mentor/échec public.                       */

for (const [thread, usages] of threadRequired) {
  const posedAt = threadPosedAt.get(thread);

  if (posedAt === undefined) {
    if (creationThreads.has(thread)) continue; // posé à la création, disponible dès 14 ans
    fail("THREAD_INCONNU", `thread '${thread}' requis par ${usages.map((u) => u.event).join(", ")} mais posé nulle part. Choix inatteignable.`);
    continue;
  }

  for (const u of usages) {
    // Inatteignable : même à la borne HAUTE de sa fenêtre, l'événement arrive avant
    // que le thread puisse exister.
    if (u.ageMax < posedAt) {
      fail(
        "SEQUENCAGE",
        `'${u.event}' (fenêtre ${u.age}-${u.ageMax}) requiert le thread '${thread}', posable au plus tôt à ${posedAt} ans. Jamais atteignable.`
      );
    } else if (u.age < posedAt) {
      // Atteignable, mais pas sur toute la fenêtre : le choix n'apparaîtra pas si
      // l'événement est tiré tôt.
      warn(
        "SEQUENCAGE_PARTIEL",
        `'${u.event}' peut être tiré dès ${u.age} ans mais '${thread}' n'existe qu'à partir de ${posedAt} ans : le choix sera absent avant cet âge.`
      );
    }
  }
}
...
for (const cb of pack.callbacks ?? []) {
  ...
  // le thread est-il réellement posé par les événements déclarés ?
  ...
  // existe-t-il un événement qui le résout, dans la bonne fenêtre d'âge ?
  ...
  // chaque branche du résolveur doit effectivement libérer le thread
...
for (const [thread, posedBy] of threadPosedBy) {
  ...
  const scoreThread = ((pack as any).scoreThreads ?? []).includes(thread);
  if (!used && !declaredCallback && !scoreThread) {
