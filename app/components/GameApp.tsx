...
import { AcademyPortrait, PortraitPicker, portraitIndex } from "@/app/components/AcademyPortrait";
import {
  InitialClubChoice,
  SeasonReview,
  TransferMarket,
    ...
    <AppFrame busy={busy} user={user} view={view} onViewChange={setView} onSignOut={supabase && user ? () => void supabase.auth.signOut() : undefined}>
      ...
      <div className="game-layout">
        <section className="game-stage"><div className="stage-content">
          ...
          {screen === "identity" && <div className="identity-flow">
            <div className="identity-heading"><div><p className="eyebrow">Ton joueur · Dernière étape</p><h1 className="display-title">Donne un visage<br />à ta légende.</h1><p className="lede">Choisis le portrait qui accompagnera toute ta carrière.</p></div><div className="identity-preview"><AcademyPortrait index={portraitIndex(selection.portrait)} label="Aperçu du joueur" /><span><small>ASCENSION ACADEMY</small><strong>{name.trim() || "Ton nom"}</strong><em>16 ans · Premier contrat</em></span></div></div>
            <PortraitPicker selected={portraitIndex(selection.portrait)} onSelect={(index) => setSelection((currentSelection) => ({ ...currentSelection, portrait: `portrait_${index}` }))} />
            <div className="identity-fields"><div className="form-field"><label htmlFor="player-name">Nom du joueur</label><input id="player-name" className="text-input" value={name} onChange={(event) => setName(event.target.value.slice(0,22))} placeholder="K. Diallo" autoComplete="off" /></div><div className="form-field"><label htmlFor="player-number">Numéro</label><input id="player-number" className="text-input" value={number} onChange={(event) => setNumber(event.target.value.replace(/\D/g,"").slice(0,2))} inputMode="numeric" /></div></div><button className="primary-button identity-continue" onClick={startCareer}>Signer mon premier contrat</button>
          </div>}
          {screen === "club_choice" && initialClubOffers.length > 0 && <InitialClubChoice offers={initialClubOffers} onChoose={chooseInitialClub} />}
          {screen === "playing" && step?.kind === "event" && <div className="premium-event"><div className="event-meta"><span>{state?.age} ans · Acte {step.event.act}</span><span>Saison {Math.max(1,(state?.season ?? 0)+1)}</span></div><div className="event-hero"><AcademyPortrait index={portraitIndex(selection.portrait)} label="Portrait du joueur" /><div className="event-hero-copy"><p className="eyebrow">Le moment du choix</p><h1 className="question">{step.event.theme ?? "Ta carrière bascule"}</h1><p className="prompt">{step.event.prompt}</p></div></div><div className="choice-list premium-choices">{step.choices.map((choice, index) => <button className={`choice-button${choice.rewarded ? " rewarded" : ""}`} key={choice.id} onClick={() => void exclusiveAction(() => choose(choice.id))}><span className="choice-index">{String(index + 1).padStart(2,"0")}</span><span>{choice.label}</span><span aria-hidden="true">›</span></button>)}</div>{adNotice && <p className="notice">{adNotice}</p>}</div>}
...
function AuthGate({ configured, onDemo, onSubmit }: { configured: boolean; onDemo: () => void; onSubmit: (email: string, marketingConsent: boolean) => Promise<string | null> }) {
  ...
  return <div className="auth-wrap"><section className="auth-card premium-home"><div className="home-visual"><AcademyPortrait index={0} label="Jeune joueur Ascension" /><span className="home-season">SAISON 2026 · LE DÉBUT DE TON HISTOIRE</span></div><div className="home-copy"><p className="eyebrow">Du premier contrat au sommet</p><h1>Écris la carrière<br />qu’on retiendra.</h1><p>Chaque choix construit ton joueur, sa réputation et sa place dans l’histoire du football.</p><div className="home-actions"><button className="primary-button" type="button" onClick={onDemo}>Commencer une nouvelle carrière</button></div><details className="account-access"><summary>Retrouver une carrière sauvegardée</summary><form onSubmit={submit}><div className="form-field"><label htmlFor="email">Adresse e-mail</label><input id="email" className="text-input" type="email" required autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="toi@exemple.fr" /></div><label className="consent"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /><span>Recevoir les nouveautés d’Ascension. Facultatif.</span></label><button className="quiet-button" disabled={!configured || status !== "idle"}>{status === "sending" ? "Envoi…" : status === "sent" ? "E-mail envoyé" : "Recevoir mon lien"}</button></form></details>{message && <p className="notice">{message}</p>}{!configured && <p className="notice">La connexion Supabase doit encore être ajoutée à cet environnement de déploiement.</p>}</div></section></div>;
}
