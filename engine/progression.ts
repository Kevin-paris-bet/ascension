import type { Event, Choice, Effects } from "./schema";
...
function mergeEffects(state: CareerState, effects: Effects): CareerState {
  ...
  if (effects.threadsResolve) {
    // Un thread peut être posé (threads) ou cumulatif (threadLevels) : la
    // résolution doit libérer les deux, sinon un callback adossé à un compteur
    // resterait ouvert pour toujours.
    for (const t of effects.threadsResolve) {
      threads.delete(t);
      delete threadLevels[t];
    }
  }
