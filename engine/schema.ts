...
export const EventSchema = z
  .object({
    ...
    /** "choice" = le joueur décide. "outcome" = écran de résultat, la branche est imposée par l'état. */
    kind: z.enum(["choice", "outcome"]).default("choice"),
  ...
  .refine(
    (e) => e.kind === "outcome" || e.choices.filter((c) => !c.requires).length >= 2,
    { message: "au moins 2 choix de base (sans requires) par événement" }
  )
  .refine(
    (e) => e.kind === "outcome" || e.choices.filter((c) => !c.requires).length <= 4,
    { message: "maximum 4 choix de base par événement" }
  )
  .refine(
    (e) => e.kind !== "outcome" || e.choices.every((c) => c.forced || c.rewarded),
    { message: "un écran 'outcome' n'a que des branches forced ou rewarded" }
  );
...
/* ---------- Création (hors événements : pas de limite de 4 options) ---------- */

export const CreationOptionSchema = z
  .object({
    id: ID,
    label: z.string().min(1),
    flavor: z.string().optional(),
    effects: EffectsSchema.optional(),
    /** Threads posés avec une probabilité, ex. { precarite_familiale: 0.6 } */
    threadChance: z.record(ID, z.number().min(0).max(1)).optional(),
  })
  .strict();
...
export const CreationStepSchema = z
  .object({
    ...
    kind: z.enum(["choice", "text", "number", "rewarded"]),
    options: z.array(CreationOptionSchema).optional(),
  })
  .strict();

export const CreationSchema = z.object({ steps: z.array(CreationStepSchema).min(1) }).strict();
...
export const ContentPackSchema = z
  .object({
    ...
    /** Threads lus par la carte finale / le score de légende, donc jamais "orphelins" */
    scoreThreads: z.array(ID).default([]),
  })
  .strict();
...
export type CreationOption = z.infer<typeof CreationOptionSchema>;
